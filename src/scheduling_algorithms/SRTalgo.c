...
// Process * sortArrayByExecTime(struct Process * array,int sizeOfArray) {
...
void SRT(int sizeOfArray ) {

    int sizeOfArray = getNbProcesses();
    Process * processes=getTableOfProcesses();
    PriorityQueue * PQ = init_priority_queue(sizeOfArray,sizeof(processes)/sizeof(processes[0]),compare_int);
    ...
    while(completed_processes<sizeOfArray) {
        ...
        if (!is_emptyPQ(pq)){
            electedProcess = pop(PQ);
            //enqueue(Q,proc)
            printf("\n Process %s is executing from : %d to : %d \n",electedProcess->processName,current_time,current_time++);
            electedProcess->runTime--;

            if (electedProcess->runTime==0){
                completed_processes++;
                printf("\n Process %s has finished executing at : %d to : %d \n",electedProcess->processName,current_time++);
            }
            else {
